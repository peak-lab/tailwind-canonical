...
| `config.ts` | `validateConfig(input)` — pure validation. Re-exports `loadConfig` from `io/`. `CONFIG_FILENAMES` (`.ts` preferred, `.js` fallback) is shared with `io/config.ts`. |
...
- `src/cli/cli.ts` — `run(argv, cwd, sink?)`, the testable CLI core (no `process.exit`; injectable `sink`). Flags: `--fix`, `--dedup`, `--merge`, `--sort`, `--analyze`, `--typos`, `--watch`, `--reporter`, `--help`/`-h`, `--version`/`-V` (help/version short-circuit before any validation, exit 0). Zero matched files → `No files matched` on stderr, exit 1. Pipeline order: fix → dedup → merge → sort. `--analyze` is a standalone mode that short-circuits the per-file pipeline and suppresses every other mode flag. `--typos` alone is a standalone scan; combined with transforms it **chains after them** (fix → dedup → merge → sort → typo scan; exit code from the typo pass — issue #96). `--watch` is only honored in transform/check mode without `--typos`. `flagWarnings(flags)` (pure, order-stable, in `cli.ts`) computes a `Warning: …` line for every ignored flag (analyze suppressions + watch-with-typos); `run` emits them via `sink.error` before dispatching — warnings never alter the active mode or exit code. `src/cli/index.ts` is a thin bin wrapper that calls `run` and `process.exit`s. Config loaded via `loadConfig` (validated). Exits 1 on findings in check mode.

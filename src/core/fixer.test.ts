...
test('fixFile', async (t) => {
  await t.test('fixes multiple arbitrary classes and returns correct count', () => {
  ...
  await t.test('ignores classes that cannot be converted', () => {
    const tempFile = join(tmpdir(), `test-${Date.now()}.tsx`)
    const content = '<div className="text-[12px] px-[7px] h-[64px]">Content</div>'
    ...
    try {
      ...
      const fixed = readFileSync(tempFile, 'utf8')
      assert.strictEqual(fixed, '<div className="text-xs px-[7px] h-16">Content</div>')
    } finally {
      unlinkSync(tempFile)
    }
  })
  ...
  await t.test('preserves whitespace and formatting', () => {
    const tempFile = join(tmpdir(), `test-${Date.now()}.tsx`)
    const content = '<div className="text-[12px]   h-[64px]   flex">Content</div>'
    ...
    try {
      ...
      const fixed = readFileSync(tempFile, 'utf8')
      assert.strictEqual(fixed, '<div className="text-xs   h-16   flex">Content</div>')
    } finally {
      unlinkSync(tempFile)
    }
  })
  ...
  await t.test('handles corner-specific rounded classes', () => {
    const tempFile = join(tmpdir(), `test-${Date.now()}.tsx`)
    const content = '<div className="rounded-tl-[12px] rounded-br-[4px]">Content</div>'
    ...
    try {
      ...
      const fixed = readFileSync(tempFile, 'utf8')
      assert.strictEqual(fixed, '<div className="rounded-tl-xl rounded-br-sm">Content</div>')
    } finally {
      unlinkSync(tempFile)
    }
  })
  ...
  await t.test('handles spacing properties with various prefixes', () => {
    ...
    try {
      ...
      assert.strictEqual(count, 4, 'Should fix all 4 spacing values (all divisible by 4)')
